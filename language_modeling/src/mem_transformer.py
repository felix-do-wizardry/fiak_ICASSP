...
import pdb
# import sys
# sys.path.append('/name/src/mixture-of-keys/lmtool-fwms/')
...
class PositionwiseFF(nn.Module):
    ...
    def forward(self, inp):
        ...
        return output
class UniformGMMAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
                 pre_lnorm=False):
        super(UniformGMMAttn, self).__init__()
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        # attn_prob = torch.einsum('ibnd,jbnd->ijbn', (head_q, head_k))
        # attn_prob.mul_(self.scale)
        attn_prob = ((-self.scale/2.0)*torch.square(torch.cdist(head_q.transpose(0,2), head_k.transpose(0,2))).permute(2, 3, 1, 0)) # n_head x bsz x qlen x klen
        # attn_prob = QK_distance0.permute(2, 3, 1, 0)
        attn_prob = 1*torch.exp(attn_prob)
        # assert 1==2
        # assert 1==2, 'hihi'
...
class GMMAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt, 
        ...
        self.update_mode = update_mode
        if self.update_mode != 'vector':
            # assert 1==2
            self.pi = nn.Parameter(torch.ones(256, 256, 1, self.n_head)/math.pow(256, 0.25), requires_grad = True) # qlen x klen x 1 x n_head
        else:
            # assert 1==2
            self.pi = nn.Parameter(torch.ones(1, 256, 1, self.n_head)/math.pow(256, 0.5), requires_grad = True)
    def forward(self, h, attn_mask=None, mems=None,
                carry_over_fast_weight=False):
        assert not carry_over_fast_weight, "Not supported."
        # multihead attention
        ...
        # 
        if self.update_mode != 'vector':
            attn_prob = self.pi_mask[:h.size(0),:c.size(0),:,:] * torch.exp(QK_distance0) # qlen x klen x bsz x n_head
        else:
            attn_prob = torch.clamp(torch.abs(self.pi), max = 1.)[:,:c.size(0),:,:] * torch.exp(QK_distance0) # qlen x klen x bsz x n_head
        # attn_prob = self.pi[:h.size(0),:c.size(0),:,:] * torch.exp(QK_distance0)
        if attn_mask is not None and attn_mask.any().item():
            if attn_mask.dim() == 2:
                attn_prob.masked_fill_(attn_mask[None,:,:,None], 0.0)
            elif attn_mask.dim() == 3:
...
class MGKAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
        ...
        if self.update_mode == 'rbf' or self.update_mode == 'rbf2keys':
            ...
            self.pi = nn.Parameter(torch.ones(256, 256, 1, self.n_head)/math.pow(256, 0.25), requires_grad = True)
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        if self.update_mode == 'hard2keys' or self.update_mode == 'soft2keys' or self.update_mode == 'rbf2keys':
            ...
            QK_distance1 = (-0.75*self.scale)*torch.square(torch.cdist(head_q.transpose(0,2), head_k1.transpose(0,2))) 
        ...
        if self.update_mode == 'hard' or self.update_mode == 'hard2keys':
            ...
            attn_prob = torch.clamp(torch.abs(self.pi), max = 1.)[:,:c.size(0),:,:] * attn_prob
    ...
    def forward(self, h, attn_mask=None, mems=None, redraw=True,
        ...
        #this normalization follows equation 29 on the paper
            ...
            if self.update_mode in ['rbf2keys','rbf']:
                ...
                # pi0 (n_head, klen)
                # combine 2 keys instead of combines 2 output results
                head_k = head_k1*self.pi0[None,:,:l,None] + head_k2*self.pi1[None,:,:l,None]
                # head_k = head_k / head_k.sum(-1, keepdim=True)
...
class MemTransformerLM(nn.Module):
    def __init__(self,
        ...
        elif attn_type in [2, 3, 4, 600]:  # absolute embeddings
        ...
        elif attn_type in [200,202]:  # absolute embeddings
            for i in range(n_layer):
                self.layers.append(
                    DecoderLayer(
                        n_head, d_model, d_head, d_inner, dropout,
        ...
        if self.attn_type == 0:  # default attention
            ...
            self.r_bias = nn.Parameter(torch.Tensor(
                    self.n_layer, self.max_klen, self.n_head))
                                ...
                                24, 25, 26,
                                34, 35,
                                44, 45, 46, 200, 300, 400, 500, 202, 600]:
    ...
    def _forward(self, dec_inp, mems=None, carry_over_fast_weight=False):
        ...
        elif self.attn_type in [2, 4, 5, 6, 7, 10, 14, 16, 200, 202, 600]:  # absolute
            if self.no_pos:
                core_out = self.drop(word_emb)
            else:
