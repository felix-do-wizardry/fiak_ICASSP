...
parser.add_argument('--load_model', action='store_true',
                    help='load model from work_dir')
parser.add_argument('--global_prune', action='store_true',
                    help='Use global pruning.')
parser.add_argument('--amount', type=float, default=0.0,
                    help='amount to entry prune')
parser.add_argument('--key_amount', type=float, default=0.0,
                    help='amount to key prune')
...
if args.use_wandb:  # configure wandb.
    ...
    wandb.init(project=project_name, entity='')
    ...
    config.global_prune = args.global_prune
    config.amount = args.amount
    config.key_amount = args.key_amount
else:
    use_wandb = False
...
if args.restart:
    ...
    print("Load model")
    model = torch.load(args.work_dir + "/model.pt")
    # model.load_state_dict(torch.load(args.work_dir + "/optimizer.pt"))
...
def train(epoch=None, para_model=para_model):
    global train_step, train_loss, best_val_loss, eval_start_time
    global log_start_time
    
    # para_model = torch.load(model_path_ft, device)
    # para_model = nn.DataParallel(para_model, dim=1).to(device)
    # model = para_model
                ...
                if not args.debug:
                    if args.global_prune:
                        with open(os.path.join(args.work_dir,
                                            f'model_FT_{int(args.amount*100)}_global.pt'), 'wb') as f:
                            torch.save(model, f)
                        with open(os.path.join(args.work_dir,
                                            f'optimizer_FT_{int(args.amount*100)}_global.pt'), 'wb') as f:
                            torch.save(optimizer.state_dict(), f)
                        print("Saved best model")

                    else:
                        with open(os.path.join(args.work_dir,
                                            f'model_FT_mixed_{int(args.key_amount*100)}_{int(args.amount*100)}_local.pt'), 'wb') as f:
                            torch.save(model, f)
                        with open(os.path.join(args.work_dir,
                                            f'optimizer_FT_mixed_{int(args.key_amount*100)}_{int(args.amount*100)}_local.pt'), 'wb') as f:
                            torch.save(optimizer.state_dict(), f)
                        print("Saved best model")
...
def prune_finetune(amount = args.amount*0.5, global_prune=global_prune):
    # Entry pruning
    ####################### GLOBAL PRUNING ###############################
    if global_prune:
        ...
        for i in range(16):
            ...
            para_model.module.layers[i].dec_attn.pi.data += torch.triu((para_model.module.layers[i].dec_attn.pi.data[:,:,0,0])*float(1000000), diagonal=1).view(256,256,1,1).expand(-1,-1,-1,8)
            parameters_to_prune.append((para_model.module.layers[i].dec_attn, 'pi'))
        ...
        for i in range(16):
            # set upper half to 0
            para_model.module.layers[i].dec_attn.pi.data *= torch.tril(torch.ones(256,256)).view(256,256,1,1).expand(-1,-1,-1,8).to(device) 

            ### remove params
            pi_mask = para_model.module.layers[i].dec_attn.pi_mask.data
            prune.remove(para_model.module.layers[i].dec_attn, 'pi')
            para_model.module.layers[i].dec_attn.register_buffer('pi_mask', pi_mask)
    ...
    ################# LOCAL PRUNING #######################
        ...
        for i in range(16):
            ...
            # Set upper half to inf, diag=1 co nghia la duong cheo khong bi thanh inf
            para_model.module.layers[i].dec_attn.pi.data += torch.triu((para_model.module.layers[i].dec_attn.pi.data[:,:,0,0])*float(100000), diagonal=1).view(256,256,1,1).expand(-1,-1,-1,8)
            # Normalization
            # pi_new = abs(para_model.module.layers[i].dec_attn.pi.data)
            # para_model.module.layers[i].dec_attn.pi.data = pi_new/(pi_new.sum(1, keepdim = True))

            # prune step
            prune.l1_unstructured(para_model.module.layers[i].dec_attn, name = 'pi', amount = amount)
    ...
    try:
        for epoch in itertools.count(start=0):
            print(f"Epoch: {epoch}")
            train(epoch=epoch, para_model=para_model)
...
def mixed_prune_finetune(amount = args.amount*0.5, key_amount=args.key_amount):
    # print("//////",torch.cuda.device_count())
    print(f'Mixed PRUNING THE MODEL WITH AMOUNT {amount*2} and key amount {key_amount}')
    ...
    for i in range(16):
        ...
        para_model.module.layers[i].dec_attn.pi.data *= torch.tril(torch.ones(256,256)).view(256,256,1,1).expand(-1,-1,-1,8).to(device) 
        # Get pi_j
        para_model.module.layers[i].dec_attn.pi_j = nn.Parameter(abs(para_model.module.layers[i].dec_attn.pi.data).sum(0, keepdim = True))
    for i in range(16):
        # Set upper to inf
        # para_model.module.layers[i].dec_attn.pi.data += torch.triu((para_model.module.layers[i].dec_attn.pi.data[:,:,0,0])*float(100000), diagonal=1).view(256,256,1,1).expand(-1,-1,-1,8)
        
        # Prune step
        prune.l1_unstructured(para_model.module.layers[i].dec_attn, name = 'pi_j', amount = key_amount)
        pi_j_mask = para_model.module.layers[i].dec_attn.pi_j_mask.data

        prune.remove(para_model.module.layers[i].dec_attn, 'pi_j')
        para_model.module.layers[i].dec_attn.register_buffer('pi_j_mask', pi_j_mask)
    print("DONE KEYS")

    # PRUNE ENTRIES
    for i in range(16):
        # Apply key mask first
        para_model.module.layers[i].dec_attn.pi.data *= para_model.module.layers[i].dec_attn.pi_j_mask.data
    for i in range(16):
        ### pruning locally layer by layer
        # Set upper half to inf
        para_model.module.layers[i].dec_attn.pi.data += torch.triu(torch.ones(256,256)).view(256,256,1,1).expand(-1,-1,-1,8).to(device)
        para_model.module.layers[i].dec_attn.pi.data += torch.triu((para_model.module.layers[i].dec_attn.pi.data[:,:,0,0])*float(100000), diagonal=1).view(256,256,1,1).expand(-1,-1,-1,8)
        # prune step
        prune.l1_unstructured(para_model.module.layers[i].dec_attn, name = 'pi', amount = amount)
...
# prune_finetune(amount = args.amount*0.5, global_prune=global_prune)
mixed_prune_finetune(amount = args.amount*0.5, key_amount=args.key_amount)
...
if args.global_prune:
    with open(os.path.join(args.work_dir, f'model_FT_{int(args.amount*100)}_global.pt'), 'rb') as f:
        model = torch.load(f)
else:
    with open(os.path.join(args.work_dir, f'model_FT_{int(args.amount*100)}_local.pt'), 'rb') as f:
        model = torch.load(f)
para_model = model.to(device)
